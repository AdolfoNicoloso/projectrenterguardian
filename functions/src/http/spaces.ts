...
export const getSpaces = onRequest(FN_OPTS, async (req, res) => {
  ...
  try {
    ...
    const data = await cms.listSpacesForProperty(ctx.appProfileId, propertyId);
...
export const createSpace = onRequest(FN_OPTS, async (req, res) => {
  ...
  try {
    ...
    const data = await cms.createSpaceForProperty(ctx.appProfileId, input);
    s.status(200).json({data});
  } catch (err: unknown) {
    sendErr(s, r, err);
  }
...
export const updateSpace = onRequest(FN_OPTS, async (req, res) => {
  ...
  try {
    ...
    const data = await cms.updateSpaceForProfile(
      ctx.appProfileId,
      String(id),
      patch
    );
...
export const deleteSpace = onRequest(FN_OPTS, async (req, res) => {
  ...
  try {
    ...
    const ok = await cms.deleteSpaceForProfile(ctx.appProfileId, input.id);
    if (!ok) {
...
export const propertyHasInspections = onRequest(FN_OPTS, async (req, res) => {
  ...
  try {
    ...
    const hasInspections = await cms.propertyHasInspections(
      ctx.appProfileId,
      propertyId
    );
